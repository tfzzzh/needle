...
class Corpus(object):
    ...
    def tokenize(self, path, max_lines=None):
        ...
        with open(path, 'r') as file:
            num_line = 0
            for line in file:
                if max_lines is not None and num_line >= max_lines:
                    break
                ...
                # add <eos> to line
                tokens.append(self.dictionary.add_word('<eos>'))
                num_line += 1
...
def batchify(data, batch_size, device, dtype):
    ...
    for i in range(batch_size):
        batches.append(data[i*nbatch : (i+1) * nbatch])

    return np.array(batches, dtype='int64').T
    ### END YOUR SOLUTION
