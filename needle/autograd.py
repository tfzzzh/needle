...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    ### BEGIN YOUR SOLUTION
    # traverse each tensor in reverse topological order
    for tensor in reverse_topo_order:
        # when the tensor do not need gradient continue:
        if not tensor.requires_grad: continue

        # compute gradient of current node via aggregate all parts
        # assert tensor.requires_grad, f"the tensor {idx} with value {tensor} shall not put in backward"
        grads_list = node_to_output_grads_list[tensor]
        tensor.grad = grads_list[0]
        for i in range(1, len(grads_list)):
            tensor.grad = tensor.grad + grads_list[i]
        
        # assert output_tensor.dtype == out_grad.dtype

        # when the node is a leaf 
        if tensor.is_leaf(): continue

        # for every input to current node propagate back gradient
        gradofinputs = tensor.op.gradient_as_tuple(tensor.grad, tensor)

        # record grad for each input
        for input, grad in zip(tensor.inputs, gradofinputs):
            if not input.requires_grad: continue
            # if input.dtype != grad.dtype:
            #     assert False
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    for input in node.inputs:
        if input not in visited: # ? shall add filter for require grad?
